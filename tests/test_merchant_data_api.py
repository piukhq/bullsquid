...
def test_create_unique_merchant(
    ...
    assert (
        Merchant.exists().where(Merchant.pk == resp.json()["merchant_ref"]).run_sync()
    )
...
def test_create_merchant_with_duplicate_name(
    ...
    plan = plan_factory.get()
    merchant = merchant_factory.get(plan=plan)
    new_merchant = merchant_factory.get(persist=False, plan=plan, name=merchant.name)
    resp = test_client.post(
        f"/api/v1/plans/{plan.pk}/merchants",
...
def test_create_merchant_with_invalid_plan_ref(
    test_client: TestClient,
    merchant_factory: ModelFactory,
    auth_header: dict,
) -> None:
